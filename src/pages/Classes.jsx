...
import { Plus, School, Trash2, X, ChevronRight, Users, Pencil, Check, AlertTriangle } from 'lucide-react'
...
const colorFromHex = (hex) => {
  const map = {
    '#00c97a': CLASS_COLORS[0],
    '#3b82f6': CLASS_COLORS[1],
    '#a855f7': CLASS_COLORS[2],
    '#f97316': CLASS_COLORS[3],
    '#ef4444': CLASS_COLORS[4],
    '#14b8a6': CLASS_COLORS[5],
    '#eab308': CLASS_COLORS[6],
    '#ec4899': CLASS_COLORS[7],
  }
  return map[hex] || CLASS_COLORS[0]
}

export default function Classes({ userRole }) {
  ...
  const addClass = () => {
    ...
    const color = colorFromHex(newColor)
  ...
  const openEdit = (cls) => {
    setEditingClass(cls)
    setNewName(cls.name)
    // find matching hex from color object
    const entry = Object.entries({ '#00c97a': 0, '#3b82f6': 1, '#a855f7': 2, '#f97316': 3, '#ef4444': 4, '#14b8a6': 5, '#eab308': 6, '#ec4899': 7 })
  ...
  if (deleteTarget) return (
    <div style={S.page}>
      ...
      <div style={{ padding: '60px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.7 }}>
            ...
            {deleteTarget.students.length > 0
              ? ` and remove all ${deleteTarget.students.length} student${deleteTarget.students.length !== 1 ? 's' : ''} in it.`
              : '.'
        ...
        <button onClick={() => setDeleteTarget(null)} style={S.secondaryBtn}>
          Cancel
        </button>
      </div>
    </div>
      ...
      {showAdd && (
        <div style={S.overlay} onClick={e => e.target === e.currentTarget && resetForm()}>
          <div style={S.sheet}>
            ...
            <button style={{ ...S.primaryBtn, marginTop: 20 }} onClick={editingClass ? saveEdit : addClass}>
        ...
        <div style={S.list}>
          {classes.map(cls => (
            <div key={cls.id} style={S.classCard}>
              {/* Color badge */}
              <div style={{ width: 46, height: 46, borderRadius: 13, display: 'grid', placeItems: 'center', fontSize: 11, fontWeight: 800, flexShrink: 0, background: cls.color.bg, color: cls.color.color, border: `1px solid ${cls.color.border}` }}>
              ...
              {canManage && (
                <div style={{ display: 'flex', gap: 6 }}>
                  <button
                    ...
                    style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--surface2)', border: '1px solid var(--border)', display: 'grid', placeItems: 'center', cursor: 'pointer', color: 'var(--muted)' }}
                  >
                    <Pencil size={14} />
                  </button>
                  <button
                    onClick={() => setDeleteTarget(cls)}
                    style={{ width: 34, height: 34, borderRadius: 10, background: '#fef2f2', border: '1px solid #fecaca', display: 'grid', placeItems: 'center', cursor: 'pointer', color: '#ef4444' }}
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
